...
useEffect(() => {
        ...
        accountService.account.subscribe(x => setAccount(x));
         Auth.currentAuthenticatedUser()
            .then(account => {console.log(account);setAccount({name:account.username});accountService.accountSubject.next({name:account.username})})
            .catch((error) => {setAccount(null); console.log('Not signed in')});
    }, []);
...
return (
    <div>
        <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
            ...
            {account &&
            <p style={{marginBottom: '10px'}}>Užívatel {account.name} bol úspešne prihlásený do rezervačného systému TK

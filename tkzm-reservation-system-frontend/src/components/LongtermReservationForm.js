...
const LongtermReservationForm = ({timeslots,reservedTimeslots, selectedTimeslot, onReservation, onEndTimeChange}) => {
    ...
    const onSubmit = (e) => {
        ...
        fetch(backendURL, requestOptions)
            ...
            error => {
                setError("Pri rezervácii dvorca sa vyskytla chyba, zadajte rezerváciu znova.");
                console.log(error)
            });
    }
    return (
                ...
                <div style={{display: 'flex', justifyContent: 'center'}}>
                    <button className="button is-rounded is-info" onClick={onSubmit}>
                        <div style={{display: 'flex', alignItems: 'center'}}>
                            ...
                            />}
                            <div
                                style={{marginLeft: '10px'}}>{TimeslotService.isSlotReserved(selectedTimeslot) ? 'Zrušiť dlhodobú rezerváciu dvorca' : 'Zadať dlhodobú rezerváciu dvorca'}</div>
                        </div>
                    </button>
                </div>
            </form>
